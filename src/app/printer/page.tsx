...
export default function PrinterPage() {
  ...
  const handleNewOrder = useCallback(async (orderId: number) => {
    ...
    if (!current) return
    if (current.order_handling_mode !== "printer") return
    if (!current.printer_connection_type) return
    ...
    try {
      ...
      const result = await printOrderTicket(
        {
        ...
        current.printer_connection_type,
        current.printer_config
      )

      if (!result.ok) {
        appendEntry({ orderId, kind: "error", message: result.error })
        return
      }

      const advance = await advanceOrderStatusAction(orderId)
      if (!advance.ok) {
        appendEntry({ orderId, kind: "error", message: `Impreso pero falló avanzar estado: ${advance.error}` })
        return
      }
      ...
      appendEntry({ orderId, kind: "error", message: "Error inesperado" })
    }
  }, [])
  ...
  const connectionOk = Boolean(restaurant.printer_connection_type)
  const ready = modeOk && connectionOk
      ...
      <div className="mx-auto max-w-3xl space-y-6">
        <header>
          ...
          <p className="mt-1 text-sm text-stone-500">
            Esta pantalla escucha los pedidos nuevos y los manda a la impresora configurada.
            Mantenela abierta en el dispositivo del local.
          </p>
        </header>
        ...
        <div className="rounded-3xl border border-stone-200 bg-white p-6 shadow-sm">
          <div className="flex items-center gap-3">
            <span
              className={`relative flex h-3 w-3 ${ready ? "" : "opacity-40"}`}
              aria-hidden="true"
            >
              <span
            ...
            <p className="text-sm font-bold text-stone-900">
              {ready ? "Listo para imprimir" : "Configuración incompleta"}
            </p>
          </div>
          ...
          <dl className="mt-5 grid gap-3 text-sm sm:grid-cols-2">
            ...
            <div>
              <dt className="text-xs font-medium uppercase tracking-wider text-stone-500">Conexión</dt>
              <dd className="mt-1 font-semibold">
                {restaurant.printer_connection_type ?? "—"}
              </dd>
            </div>
          </dl>
          ...
          {!ready && (
            <p className="mt-5 rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-xs font-medium text-amber-800">
              Configurá la impresora desde <span className="font-mono">/admin/settings</span>.
            </p>
          )}
        </div>
