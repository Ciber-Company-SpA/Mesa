-- ============================================================================
-- create_public_order_qr con CUPÓN (p_coupon_code). Recrea la función del paso
-- de promociones (20260720250000) sumando, al final, la aplicación del cupón:
-- valida el cupón (reglas + vigencia hora de Chile), calcula el descuento sobre
-- el alcance (toda la carta / categoría / producto) EXCLUYENDO las líneas de
-- promoción, respeta el mínimo de compra, asienta orders.discount_amount y el
-- total final, e incrementa used_count. Cambia la firma (agrega p_coupon_code),
-- así que primero se elimina la versión de 3 args.
-- ============================================================================

drop function if exists public.create_public_order_qr(text, jsonb, text);
...
begin
  ...
  if p_coupon_code is not null and length(trim(p_coupon_code)) > 0 then
    ...
    if found then
      -- Base elegible: siempre excluye las líneas de promoción (promotion_id null).
