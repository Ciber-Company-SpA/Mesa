-- get_top_products_today acepta ahora `p_from` (start-of-day en zona horaria
-- local del cliente) para evitar el desfase con UTC. Si no se pasa, fallback
-- al inicio del día UTC (comportamiento anterior).
...
AS $$
  ...
  FROM (
    ...
    WHERE o.restaurant_id = p_restaurant_id
      AND o.status_id = 4
      AND o.created_at >= COALESCE(p_from, date_trunc('day', now()))
      AND p.status_id = 1
    GROUP BY
...
GRANT EXECUTE ON FUNCTION public.get_top_products_today(bigint, int, timestamptz) TO anon, authenticated, service_role;
