-- ============================================================================
-- DESCUENTO DE STOCK EN create_public_order_qr
--
-- Recrea la RPC vigente (fase 3: con rate limit) añadiendo, dentro del loop de
-- líneas, el descuento de stock por receta:
--   - Resuelve la receta del nivel correcto: variante si la línea trae
--     variant_id; si no, la del producto.
--   - Bloquea las filas de insumo (FOR UPDATE) para evitar sobreventa cuando
--     entran dos pedidos a la vez.
--   - BLOQUEO DURO: si algún insumo no alcanza, lanza excepción nombrando el
--     insumo y el producto. Toda la transacción se revierte (no se descuenta ni
--     se crea la orden a medias).
--   - Registra un movimiento 'venta' por insumo; el trigger mantiene el saldo y
--     recalcula la disponibilidad (stock_out) de los productos afectados.
--
-- Productos SIN receta no consumen stock (se venden libre).
-- ============================================================================
...
begin
  ...
  -- RATE LIMIT (BD): máx 15 pedidos por 60s por mesa.
  ...
  loop
    ...
    -- ----------------------------------------------------------------------
    -- DESCUENTO DE STOCK POR RECETA (bloqueo duro).
    -- Receta de la variante si la línea trae variant_id; si no, del producto.
    -- Sin receta => no consume stock. FOR UPDATE evita sobreventa concurrente.
    -- ----------------------------------------------------------------------
    for v_recipe in
      select r.ingredient_id, r.cantidad, i.name as ing_name, i.stock_actual
    ...
    loop
      ...
      if v_recipe.stock_actual < v_needed then
        raise exception 'Sin stock de "%" para preparar "%"',
          v_recipe.ing_name, coalesce(v_variant_name, v_product_name);
      end if;
